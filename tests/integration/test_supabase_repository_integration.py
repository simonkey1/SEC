...
class TestGetOrCreateGeografia:
    ...
    def test_creates_new_geografia_when_not_exists(self, repo):
        ...
        # Cleanup
        repo.supabase.table("dim_geografia").delete().eq(
            "id_geografia", id_geo
        ).execute()
...
class TestSaveRecords:
    ...
    def test_inserts_new_records_successfully(self, repo):
        ...
        repo.supabase.table("fact_interrupciones").delete().eq(
            "hash_id", "test_hash_12345"
        ).execute()
        repo.supabase.table("dim_geografia").delete().eq(
            "nombre_region", "TEST_REGION"
        ).execute()
        repo.supabase.table("dim_empresa").delete().eq(
            "nombre_empresa", "TEST_EMPRESA"
        ).execute()
        repo.supabase.table("dim_tiempo").delete().eq(
            "id_tiempo", 202601231430
        ).execute()
    ...
    def test_handles_duplicate_records(self, repo):
        ...
        # Cleanup
        repo.supabase.table("fact_interrupciones").delete().eq(
            "hash_id", "duplicate_hash_test_99999"
        ).execute()
