...
class HealthChecker:
    ...
    def check_sec_health(self) -> dict:
        ...
        try:
            ...
            logger.error(
                f"SEC health check failed (attempt {self.failed_attempts}): {e}"
            )
    ...
    def log_execution(self, execution_data: dict):
        ...
        logger.info(
            f"Execution: {execution_data['status']}, {execution_data['rows_transformed']} rows"
        )
...
def main():
    ...
    while True:
        ...
        try:
            ...
            transformed, failed = SecDataTransformer.transform(
                raw_data, health["hora_servidor"]
            )
